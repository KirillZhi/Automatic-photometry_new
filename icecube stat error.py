...
syst_err_arr = np.arange(0., 5., 0.2)
best_syst_err_arr = 0
min_sq_sum = 10 ** 6
for syst_ind in range(len(syst_err_arr)):
    ...
    for i in range(len(one_sigma_dist)):
        cum_distr_fun[i] = (i + 1.) / len(one_sigma_dist)
        distribution_function_ninety_perc_conf[i] = math.sqrt(i + 1.) / len(one_sigma_dist) * 1.65
        value_num = 0
    ...
    if syst_ind == 50:
        ...
        plt.fill_between(new_source_dist, cum_distr_fun - distribution_function_ninety_perc_conf,
...
for i in range(len(one_sigma_dist)):
    test_cum_distr_fun[i] = (i + 1.) / len(one_sigma_dist)
    test_distribution_function_ninety_perc_conf[i] = math.sqrt(i + 1.) / len(one_sigma_dist) * 1.65
    value_num = 0
    for j in range(len(distribution_function)):
        if i != len(one_sigma_dist) - 1 and i != 0:
...
line1 = plt.plot(r_values, distribution_function, c='r')
line2 = plt.plot(new_source_dist, cum_distr_fun, c='b')
plt.fill_between(new_source_dist, cum_distr_fun - distribution_function_ninety_perc_conf, cum_distr_fun + distribution_function_ninety_perc_conf, color='b', alpha = .1)
plt.show()
plt.close()
