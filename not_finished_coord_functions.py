...
def adv_min_fun(exp_x, exp_y, theoretical_x, theoretical_y, parameters, devia_x, devia_y):  # Function we minimize
    init_fun = 0
    f_fun = 0
    g_fun = 0
    for fun_ind in range(len(exp_y)):
        f_fun = exp_x[fun_ind] - parameters[3] * theoretical_x[fun_ind] * cos(parameters[2]) - parameters[3] * theoretical_y[fun_ind] * sin(
            parameters[2]) - parameters[0] * cos(parameters[2]) - parameters[1] * sin(parameters[2]) - devia_x[fun_ind]\
                * cos(parameters[2]) * parameters[4] - devia_y[fun_ind] * sin(parameters[2]) * parameters[4]
        g_fun = exp_y[fun_ind] + parameters[3] * theoretical_x[fun_ind] * sin(parameters[2]) - parameters[3] * theoretical_y[fun_ind] * cos(
            parameters[2]) + parameters[0] * sin(parameters[2]) - parameters[1] * cos(parameters[2]) + devia_x[fun_ind]\
                * sin(parameters[2]) * parameters[4] - devia_y[fun_ind] * cos(parameters[2]) * parameters[4]
        init_fun += f_fun ** 2 + g_fun ** 2
    del f_fun, g_fun
    return init_fun

def adv_fun_min_search(input_x, input_y, cel_x, cel_y, init_teta, devia_x, devia_y):  # Search for a minimum in coordinate deviation
    min = 100000000
    ...
    if init_teta == 1000:
        ...
        for teta_step_ind in range(len(teta_step)):
            ...
            for step_ind in range(0, teta_steps):
                ...
                # print(input_x, x_transform, input_y)
                x_transform_sum = sum(x_transform)
                y_transform_sum = sum(y_transform)
                x_devia_zen = devia_x
                y_devia_zen = devia_y
                x_devia_zen_sum = sum(x_devia_zen)
                y_devia_zen_sum = sum(y_devia_zen)
                anchor_num = len(cel_x)
                A_arr = np.array([
                    [anchor_num, 0, x_cel_sum, x_devia_zen_sum],
                    [0, anchor_num, y_cel_sum, y_devia_zen_sum],
                    [x_devia_zen_sum, y_devia_zen_sum, sum(cel_x * x_devia_zen + cel_y * y_devia_zen),
                     sum(x_devia_zen ** 2 + y_devia_zen ** 2)],
                    [x_cel_sum, y_cel_sum, sum(cel_x ** 2 + cel_y ** 2),
                     sum(cel_x * x_devia_zen + cel_y * y_devia_zen)]], dtype=float)
                # print(A_arr, np.linalg.det(A_arr))
                # print(A_arr.shape)
                Y_arr = np.array([[x_transform_sum],
                                  [y_transform_sum],
                                  [sum(input_x * (x_devia_zen * cos(teta_value) + y_devia_zen * sin(
                                      teta_value)) - input_y * (x_devia_zen * sin(
                                      teta_value)))],
                                  [sum(input_x * (cel_x * cos(teta_value) + cel_y * sin(teta_value)) - input_y * (
                                              cel_x * sin(teta_value) - cel_y * cos(teta_value)))]])
                ...
                frame_inter_parameters[4] = X_arr[3]
                # Determining other parameters
                function_value = adv_min_fun(input_x, input_y, cel_x, cel_y, frame_inter_parameters, devia_x, devia_y)
        ...
        teta_step = [3.75, 1.875, 0.938, 0.469, 0.234, 0.117, 0.0585, 0.02925, 0.014625, 0.0073]
        ...
        for teta_step_ind in range(len(teta_step)):
            ...
            for step_ind in range(0, teta_steps):
                ...
                x_cel_sum = sum(cel_x)
                y_cel_sum = sum(cel_y)
                teta_arr = np.arange(0, 2 * 3.14159, 0.0005)
                function_arr = np.zeros_like(teta_arr)
                for teta_ind in range(0):
                    teta_value = teta_arr[teta_ind]
                    x_transform = input_x * cos(teta_value) - input_y * sin(teta_value)
                    y_transform = input_x * sin(teta_value) + input_y * cos(teta_value)
                    # print(input_x, x_transform, input_y)
                    x_transform_sum = sum(x_transform)
                    ...
                    # X_arr = np.dot(A_inv, Y_arr)
                    # Gauss elimination
                    inter_param = np.zeros(4)
                    x_converted_arr, y_converted_arr = gauss_elimination(np.dot(np.transpose(A_arr), A_arr),
                                                                         np.dot(np.transpose(A_arr), Y_arr))
                    for parameter_ind in range(x_converted_arr.shape[0] - 1, -1, -1):
                        ...
                        if parameter_ind != x_converted_arr.shape[0] - 1:
                            for substract_ind in range(parameter_ind + 1, x_converted_arr.shape[1]):
                                substract_value += inter_param[substract_ind] * x_converted_arr[parameter_ind][
                                    substract_ind]
                        right_value = y_converted_arr[parameter_ind] - substract_value
                        inter_param[parameter_ind] = right_value / x_converted_arr[parameter_ind][parameter_ind]
                    frame_inter_parameters[0] = inter_param[0]
                    frame_inter_parameters[1] = inter_param[1]
                    frame_inter_parameters[2] = teta_value
                    frame_inter_parameters[3] = inter_param[2]
                    frame_inter_parameters[4] = inter_param[3]
                    # frame_inter_parameters[0] = 516.0
                    # frame_inter_parameters[1] = -506.0
                    # frame_inter_parameters[3] = 0
                    # Determining other parameters
                    function_arr[teta_ind] = adv_min_fun(input_x, input_y, cel_x, cel_y, frame_inter_parameters, devia_x,
                                                 devia_y)
                #plt.plot(teta_arr, function_arr)
                #plt.show()
                #plt.close()
                x_transform = input_x * cos(teta_value) - input_y * sin(teta_value)
                y_transform = input_x * sin(teta_value) + input_y * cos(teta_value)
                ...
                A_inv = np.linalg.inv(A_arr + alpha * np.identity(4))

                X_arr = np.dot(A_inv, Y_arr)

                # Gauss elimination
                #inter_param = np.zeros(4)
                #x_converted_arr, y_converted_arr = gauss_elimination(np.dot(np.transpose(A_arr), A_arr), np.dot(np.transpose(A_arr), Y_arr))
                #for parameter_ind in range(x_converted_arr.shape[0] - 1, -1, -1):
                #    substract_value = 0
                #    if parameter_ind != x_converted_arr.shape[0] - 1:
                #        for substract_ind in range(parameter_ind + 1, x_converted_arr.shape[1]):
    ...
    del teta_step_ind, A_arr, Y_arr, x_transform, y_transform
...
def fun_min_search(input_x, input_y, cel_x, cel_y, init_teta):  # Search for a minimum in coordinate deviation
    ...
    if init_teta == 1000:
        ...
        for teta_step_ind in range(len(teta_step)):
            angle_step = teta_step[teta_step_ind]
            teta_lower = teta_min - angle_step * 2
            teta_upper = teta_min + angle_step * 2
            teta_steps = int((teta_upper - teta_lower) / teta_step[teta_step_ind])
...
def axis_fit_new(input_x, input_y, mid_ra, mid_dec, input_ra, input_dec, input_parameters):
    ...
    #print('abasdsa', input_parameters[5])
    axis_direction = input_parameters[5]
...
def adv_axis_fit_complete(input_x, input_y, mid_ra, mid_dec, input_ra, input_dec,
                      input_parameters, date_time, obs_lat, obs_lon):  # Fitting distortion and rotation
    # input_x, input_y - x and y coordinate on the frame (distorted)
    # frame_path - path to a frame
    # input_ra, input_dec - corresponding celestial coordinates
    # axis_direction - check if a frame is mirrored in some way
    ...
    y_cel_use = copy(y_cel)
    #print(x_cel)
    zen0, azim0 = zen_dist_sing(obs_lon, obs_lat, date_time, mid_ra, mid_dec)

    zen_arr = zen_dist(obs_lon, obs_lat, date_time, input_ra, input_dec)
    devia_x0, devia_y0 = devia_const_sing(mid_ra, mid_dec, zen0, mid_ra, mid_dec, azim0, zen0, obs_lat)
    devia_x_arr, devia_y_arr = devia_const(input_ra, input_dec, zen_arr, mid_ra, mid_dec, azim0, zen0, obs_lat)

    non_distorted_parameters, non_distorted_sum = adv_axis_fit_nondistorted(input_x, input_y, x_cel_use, y_cel_use, input_parameters, devia_x_arr, devia_y_arr)
...
def adv_axis_fit_use(input_ra, input_dec, mid_ra, mid_dec, frame_parameters, date_time, obs_lat, obs_lon):
    # Making a celestial projection array
    x_cel = 0
    y_cel = 0
    x_cel = rad_conversion * (
            ...
            mid_dec / rad_conversion))
    y_cel = rad_conversion * (
                sin((input_ra - mid_ra) / rad_conversion) * cos(input_dec / rad_conversion))
    zen0, azim0 = zen_dist_sing(obs_lon, obs_lat, date_time, mid_ra, mid_dec)
    zen_arr = zen_dist(obs_lon, obs_lat, date_time, input_ra, input_dec)
    devia_x_arr, devia_y_arr = devia_const(input_ra, input_dec, zen_arr, mid_ra, mid_dec, azim0, zen0, obs_lat)
    # Finding coordinates
    if frame_parameters[5] == 1:
        x_cel = x_cel * (-1)
        devia_x_arr = devia_x_arr * (-1)
    elif frame_parameters[5] == 2:
        y_cel = y_cel * (-1)
        devia_y_arr = devia_y_arr * (-1)
    elif frame_parameters[5] == 3:
        y_cel = y_cel * (-1)
        x_cel = x_cel * (-1)
        devia_x_arr = devia_x_arr * (-1)
        devia_y_arr = devia_y_arr * (-1)

    x_theor = frame_parameters[3] * (x_cel * cos(frame_parameters[2]) + y_cel * sin(frame_parameters[2]))
    y_theor = frame_parameters[3] * (-x_cel * sin(frame_parameters[2]) + y_cel * cos(frame_parameters[2]))
    x_output = x_theor + frame_parameters[0] * cos(frame_parameters[2]) + frame_parameters[1] * sin(frame_parameters[2]) + devia_x_arr * cos(frame_parameters[2]) * frame_parameters[4] + devia_y_arr * (zen_arr - zen0) * sin(frame_parameters[2]) * frame_parameters[4]
    y_output = y_theor - frame_parameters[0] * sin(frame_parameters[2]) + frame_parameters[1] * cos(frame_parameters[2]) - devia_x_arr * sin(frame_parameters[2]) * frame_parameters[4] + devia_y_arr * (zen_arr - zen0) * cos(frame_parameters[2]) * frame_parameters[4]
    #x_output = x_theor + frame_parameters[0] * cos(frame_parameters[2]) + frame_parameters[1] * sin(
    #    frame_parameters[2])
    #y_output = y_theor - frame_parameters[0] * sin(frame_parameters[2]) + frame_parameters[1] * cos(
    #    frame_parameters[2])

    del x_theor, y_theor, x_cel, y_cel, mid_ra, mid_dec
    return int(round(x_output)), int(round(y_output))

def adv_axis_fit_nondistorted(input_x, input_y, cel_x, cel_y, input_parameters, devia_x, devia_y):  # Fitting only rotation, shift and scale
    min_boundary = 2 * len(input_x) * 15 ** 2
    axis_direction = input_parameters[5]
    ...
    if axis_direction == 5:
        for axis_direction in range(4):
            ...
            if axis_direction == 1:
                ...
                devia_x = devia_x * (-1)
                devia_y = devia_x * (-1)
            min_value, inter_parameters = adv_fun_min_search(input_x, input_y, x_cel_inter, y_cel_inter, 1000, devia_x, devia_y)
        ...
        min_value, inter_parameters = adv_fun_min_search(input_x, input_y, x_cel_inter, y_cel_inter, input_parameters[2], devia_x, devia_y)
        x_cel_final = copy(x_cel_inter)
        y_cel_final = copy(y_cel_inter)
        min_axis = axis_direction
        inter_parameters[5] = min_axis
...
def quad_frame(coord_x, coord_y):
    ...
    quad_x_1_err, quad_y_1_err = quad_coord_err(max_len, coord_c, coord_a, diag_pos_angle, 10)
    quad_x_2_err, quad_y_2_err = quad_coord_err(max_len, coord_d, coord_a, diag_pos_angle, 10)
...
def gaia_bright(catalog_path, anchor_num, offset, radius, obj_ra, obj_dec, check_up, anchor_up, mag_low, source_mag_int, saturation_mag):
    center_ra = obj_ra
    center_dec = obj_dec
    ...
    #print(data)
    if saturation_mag >=0:
        data_bright = data.loc[(data['Gmag'] <= (saturation_mag + 2.5)) & (data['Gmag'] >= saturation_mag)]
    else:
        data_bright = pd.DataFrame.copy(data)
    if offset >=2:
        data_bright = data_bright.nsmallest(30, 'Gmag')[offset - 2:offset + 3 * anchor_num]  # The brightest with an offset
    else:
        data_bright = data_bright.nsmallest(30, 'Gmag')[:3 * anchor_num]
    #print('bbb')
    #print(data_bright)
    ...
    super_bright = data.loc[(data['Gmag'] <= 11.5)]  # The brightest stars
    ...
    anchor_stars = inter_bright.loc[(inter_bright['Gmag'] <= mag_low) & (inter_bright['Gmag'] >= anchor_up)]
    if len(anchor_stars['Gmag']) >=100:
        anchor_stars = anchor_stars.nsmallest(100, 'Gmag')
    print("Number of initial anchors: " + "{}".format(len(anchor_stars)))
    stars_for_checking = data.loc[(data['Gmag'] <= mag_low + 2) & (data['Gmag'] >= saturation_mag)]
    ...
    for mag_ind in range(len(mag_limits)):
        ...
        try:
            ...
            print('not enought checks for a magnitude interval')
    #print('ddd')
    check_stars = pd.DataFrame(data=check_dict, columns=array_columns)
    #print(check_stars)
    ...
    for anchor_ind in range(len(anchor_stars['Gmag'])):
        ...
        for intermediate_ind in range(len(stars_for_checking['Gmag'])):
            ...
            if ang_sep <= 0.2 / 60 / rad_conversion and ang_sep != 0 and mag_diff <= 3:
                adding_check = 1
                #print('cccc')
                break
        if adding_check == 0:
...
def quad_cel(input_RA, input_DEC):
    ...
    # Positional angles for inner stars
    #len1 = sin(Angle(angular_separation(coord_RA[inner_index[0]], coord_DEC[inner_index[0]], max_RA_1, max_DEC_1), u.rad).wrap_at('360d').rad)
    #len2 = sin(Angle(angular_separation(coord_RA[inner_index[1]], coord_DEC[inner_index[1]], max_RA_1, max_DEC_1), u.rad).wrap_at('360d').rad)
...
# Function for finding zenith distance for given coordinates and time

def zen_dist_sing(obs_long, obs_lat, obs_datetime, obj_ra, obj_dec):
    obs_location = EarthLocation(lat=obs_lat * u.deg, lon=obs_long * u.deg)
    obs_time = Time(obs_datetime, format='isot', scale='utc', location=obs_location)
    LST = obs_time.sidereal_time('mean').deg
    time_angle = (LST - obj_ra) / rad_conversion
    zenith_distance = math.acos(sin(obs_lat / rad_conversion) * sin(obj_dec / rad_conversion) + cos(obs_lat / rad_conversion) * cos(obj_dec / rad_conversion) * cos(time_angle))
    azimuth = math.atan2(cos(obj_dec / rad_conversion) * cos(time_angle), - cos(obs_lat / rad_conversion) * sin(obj_dec / rad_conversion) + sin(obs_lat / rad_conversion) * cos(obj_dec / rad_conversion) * cos(time_angle))
    return zenith_distance * rad_conversion, azimuth

def zen_dist(obs_long, obs_lat, obs_datetime, obj_ra, obj_dec):
    obs_location = EarthLocation(lat=obs_lat * u.deg, lon=obs_long * u.deg)
    ...
    if type(obj_ra) is float:
        zenith_distance = math.acos(
            sin(obs_lat / rad_conversion) * sin(obj_dec / rad_conversion) + cos(obs_lat / rad_conversion) * cos(
                obj_dec / rad_conversion) * cos(time_angle))
    else:
        zenith_distance = np.arccos(
            np.sin(obs_lat / rad_conversion) * np.sin(obj_dec / rad_conversion) + np.cos(obs_lat / rad_conversion) * np.cos(
                obj_dec / rad_conversion) * np.cos(time_angle))
    return zenith_distance * rad_conversion

def devia_const_sing(obj_ra, obj_dec, obj_zen, mid_ra, mid_dec, mid_az, mid_zen, obs_lat):
    gamma_const = (sin(obs_lat / rad_conversion) * sin(mid_zen / rad_conversion) + cos(obs_lat / rad_conversion) * cos(mid_zen / rad_conversion) * cos(mid_az)) / cos(mid_dec / rad_conversion)
    #devia_y = gamma_const * rad_conversion * (((sin(obj_dec / rad_conversion) * cos(mid_dec / rad_conversion) * cos((obj_ra - mid_ra) / rad_conversion) + cos(obj_dec / rad_conversion) * cos(mid_dec / rad_conversion)) * obj_zen + (cos(obj_dec / rad_conversion) * sin(mid_dec / rad_conversion) * cos((obj_ra - mid_ra) / rad_conversion) - sin(obj_dec / rad_conversion) * sin(mid_dec / rad_conversion)) * mid_zen))
    devia_x = gamma_const * rad_conversion * ((sin(obj_dec / rad_conversion) * sin(mid_dec / rad_conversion) * cos((obj_ra - mid_ra) / rad_conversion) + cos(obj_dec / rad_conversion) * cos(mid_dec / rad_conversion)))


    devia_y = gamma_const * rad_conversion * (sin(obj_dec / rad_conversion) * sin((obj_ra - mid_ra) / rad_conversion))
    return devia_x, devia_y

def devia_const(obj_ra, obj_dec, obj_zen, mid_ra, mid_dec, mid_az, mid_zen, obs_lat):
    gamma_const = (sin(obs_lat / rad_conversion) * sin(mid_zen / rad_conversion) + cos(obs_lat / rad_conversion) * cos(mid_zen / rad_conversion) * cos(mid_az)) / cos(mid_dec / rad_conversion)
    if type(obj_ra) is float:
        devia_x = gamma_const * rad_conversion * (sin(obj_dec / rad_conversion) * sin(mid_dec / rad_conversion) * (mid_zen - obj_zen * cos((obj_ra - mid_ra) / rad_conversion)) + cos(obj_dec / rad_conversion) * cos(mid_dec / rad_conversion) * (mid_zen * cos((obj_ra - mid_ra) / rad_conversion) - obj_zen))

        devia_y = gamma_const * rad_conversion * (
                    sin(obj_dec / rad_conversion) * sin((obj_ra - mid_ra) / rad_conversion))
    else:
        devia_x = gamma_const * rad_conversion * (np.sin(obj_dec / rad_conversion) * np.sin(mid_dec / rad_conversion) * (mid_zen - obj_zen * np.cos((obj_ra - mid_ra) / rad_conversion)) + np.cos(obj_dec / rad_conversion) * np.cos(mid_dec / rad_conversion) * (mid_zen * np.cos((obj_ra - mid_ra) / rad_conversion) - obj_zen))
        devia_y = gamma_const * rad_conversion * (np.sin(obj_dec / rad_conversion) * np.sin((obj_ra - mid_ra) / rad_conversion)) * obj_zen
    return devia_x, devia_y