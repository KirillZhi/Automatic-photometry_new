...
def adaptive_photometry_source(x0, y0, ground, frame_equiv_gain):  # Photometry with an automatic choice of aperture parameters(max SNR)
    max_check = 0
    ext_radius = 20
    gap = 5
    inn_radius = 1
    ...
    while max_check == 0:
        ...
        source_err_arr = np.sqrt(source_arr * frame_equiv_gain + readout_noise ** 2)
        SNR_arr = (source_arr - background_per_pixel) / source_err_arr
        one_point_five_sigma_pixels_arr = np.count_nonzero(SNR_arr >= 1.5)
        #print(source_arr)
        #print(one_point_five_sigma_pixels_arr, source_arr.shape, 'aaa')
        if initial_check == 0:
            if float(one_point_five_sigma_pixels_arr) / (source_arr.shape[0]) <= 0.5:
                output_inn_radius = inn_radius
                break
        source_signal = total_signal - background_per_pixel * source_pixel
        SNR = source_signal / math.sqrt(total_signal * frame_equiv_gain + readout_noise ** 2 * source_pixel)
        #print(SNR, inn_radius)
    ...
    background_per_pixel = astropy.stats.sigma_clipped_stats(background_arr, sigma=1)[0]
    source_signal = total_signal - background_per_pixel * source_pixel
    source_err = math.sqrt(total_signal * frame_equiv_gain + readout_noise ** 2 * source_pixel)

    # Calculating a 3 sigma limit
    snr_limit = 3
    #source_pixel = 5
    signal_limit = (snr_limit ** 2 + sqrt(snr_limit ** 4 + 4 * source_pixel * snr_limit ** 2 * (background_per_pixel * frame_equiv_gain + readout_noise ** 2))) / 2
    return source_signal, source_err, output_inn_radius, signal_limit


def photometry_source(x0, y0, inn_radius, intermediate_increment, ext_radius, ground, frame_equiv_gain):
    source_pixel = 0
    background_arr = []
    total_signal = 0
    x0 = int(round(x0))
...
def photometry_snr(x0, y0, inn_radius, intermediate_increment, ext_radius, ground, back_level, frame_equiv_gain):
    source_pixel = 0
    background_arr = []
    total_signal = 0
    x0 = int(round(x0))
...
def prec_photometry_snr(x0, y0, inn_radius, intermediate_increment, ext_radius, ground, frame_equiv_gain):
    source_pixel = 0
    background_arr = []
    total_signal = 0
    x0 = int(round(x0))
    ...
    for i in range(x_bottom, x_top):
        for j in range(y_bottom, y_top):
            ...
            if ground[i][j] >= peak_brightness:
                peak_brightness = ground[i][j]
    return peak_brightness

def photometry_source_ell(x0, y0, inn_radius, gap, annul1, annul2, angle, ground, frame_equiv_gain):
    source_signal = 0
    source_pixel = 0
    ...
    # if source_signal<0:
    # source_signal=
    SNR = source_signal / math.sqrt(total_signal * frame_equiv_gain)
    source_err = source_signal / SNR
    # print(source_pixel)
    return source_signal, source_err


def photometry_ell_source(x0, y0, inn_radius2, inn_radius1, intermediate_increment, ext_radius, ground, frame_equiv_gain):
    source_signal = 0
    source_pixel = 0
    background_per_pixel = 0
    background_arr = []
...
def photometry_check(x0_arr, y0_arr, inn_radius, intermediate_increment, ext_radius, ground, frame_equiv_gain):
    x0_arr = np.int64(np.round(x0_arr))
    y0_arr = np.int64(np.round(y0_arr))
    # print(x0_arr)
    frame_mid_x = ground.shape[0] / 2
    frame_mid_y = ground.shape[1] / 2
    source_signal = np.zeros_like(x0_arr, dtype=float)
    source_signal_err = np.zeros_like(x0_arr, dtype=float)
    inn_radius = int(inn_radius)
    intermediate_increment = int(intermediate_increment)
    ext_radius = int(ext_radius)
    max_brightness = np.max(ground)
    if max_brightness >= 2 * 10 ** 5:
        max_brightness = 64000
    for k in range(len(x0_arr)):
        oversaturation_check = 0
        if abs(x0_arr[k] - frame_mid_x) >= frame_mid_x * 0.9 or abs(y0_arr[k] - frame_mid_y) >= frame_mid_y * 0.9:
            source_signal[k] = np.nan
            source_signal_err[k] = np.nan
            continue
        if ground[x0_arr[k]][y0_arr[k]] == 0:
            source_signal[k] = np.nan
            source_signal_err[k] = np.nan
            continue
        source_pixel = 0
        ...
        for i in range(x_bottom, x_top):
            for j in range(y_bottom, y_top):
                rad_val = math.sqrt(((x0_arr[k] - i) ** 2 + (y0_arr[k] - j) ** 2))
                if ground[i][j] >= max_brightness * 0.95 and rad_val <= inn_radius:
                    oversaturation_check = 1
                    break
                if ground[i][j] > 0:
                    if rad_val <= inn_radius:
                        total_signal += ground[i][j]
                    ...
                    if rad_val > inn_radius + intermediate_increment and rad_val <= ext_radius + 1:
                        background_arr = np.append(background_arr, ground[i][j])
            if oversaturation_check == 1:
                break
        if oversaturation_check == 0:
            ...
            source_signal_err[k] = math.sqrt(total_signal * frame_equiv_gain + readout_noise ** 2 * source_pixel)
        else:
            source_signal[k] = np.nan
            source_signal_err[k] = np.nan
    return source_signal, source_signal_err

def photometry_test(x0, y0, inn_radius, intermediate_increment, ext_radius, ground, frame_equiv_gain):
    # print(x0_arr)
    x0 = int(x0)
    y0 = int(y0)
    frame_mid_x = ground.shape[0] / 2
    ...
    oversaturation_check = 0
    if abs(x0 - frame_mid_x) >= frame_mid_x * 0.9 or abs(y0 - frame_mid_y) >= frame_mid_y * 0.9:
        source_signal = np.nan
        source_signal_err = np.nan
    if ground[x0][y0] == 0:
        source_signal = np.nan
        source_signal_err = np.nan
    source_pixel = 0
    ...
    for i in range(x_bottom, x_top):
        for j in range(y_bottom, y_top):
            rad_val = math.sqrt(((x0 - i) ** 2 + (y0 - j) ** 2))
            if ground[i][j] >= max_brightness * 0.95:
                print('alarm1', max_brightness, ground[i][j], i, j)
                oversaturation_check = 1
                break
            if ground[i][j] > 0:
                if rad_val <= inn_radius:
    ...
    if oversaturation_check == 0:
        ...
        source_signal = total_signal - background_per_pixel * source_pixel
            ...
            #print(total_signal)
        #print(source_signal[k])
        source_signal_err = math.sqrt(total_signal * frame_equiv_gain + readout_noise ** 2 * source_pixel)
    else:
        source_signal = np.nan
        source_signal_err = np.nan
    print(oversaturation_check, 'dddddddddddddddddddddddd')
    return source_signal, source_signal_err

def photometry_check_ref(x0_arr, y0_arr, inn_radius, intermediate_increment, ext_radius, ground, frame_equiv_gain):
    x0_arr = np.int64(np.round(x0_arr))
    y0_arr = np.int64(np.round(y0_arr))
    # print(x0_arr)
    source_signal = np.zeros_like(x0_arr, dtype=float)
    ...
    for k in range(len(x0_arr)):
        ...
        source_signal[k] = total_signal - background_per_pixel * source_pixel
            ...
            #print(total_signal)
        #print(source_signal[k])
        source_signal_err[k] = float(math.sqrt(total_signal * frame_equiv_gain + readout_noise ** 2 * source_pixel))
    return source_signal, source_signal_err


# Calculation of magnitude and its variation
def magnitude(mag_ref, photo_ref, photo_source, photo_source_err, photo_check, photo_check_err):
    mag_source = np.zeros_like(photo_ref)
    mag_check = np.zeros_like(photo_check)
    mag_source_err = np.zeros_like(photo_ref)
    for frame_ind in range(len(mag_ref)):
        if photo_source[frame_ind] / photo_source_err[frame_ind] >= 3.:
            mag_source[frame_ind] = mag_ref[frame_ind] - 2.5 * math.log10(photo_source[frame_ind] / photo_ref[frame_ind])
            mag_source_err[frame_ind] = float(photo_source_err[frame_ind]) / photo_source[frame_ind]
        else:
            mag_source[frame_ind] = np.nan
            mag_source_err[frame_ind] = np.nan
    #print(np.log10(photo_source / photo_ref))
    for k in range(photo_check.shape[1]):
        for j in range(photo_check.shape[0]):
            if math.isnan(photo_check[j][k]):
                mag_check[j][k] = np.nan
            else:
                if photo_check[j][k] / photo_check_err[j][k] >= 3:
                    mag_check[j][k] = mag_ref[j] - 2.5 * math.log10(photo_check[j][k] / photo_ref[j])
                else:
                    mag_check[j][k] = np.nan

    return mag_source, mag_source_err, mag_check

def magnitude_check(mag_ref, photo_ref, photo_check, photo_check_err):
    mag_check = np.empty_like(photo_check, dtype=float)
    # print(np.log10(photo_source / photo_ref))
    mag_check[:] = np.nan
    for k in range(photo_check.shape[1]):
        for j in range(photo_check.shape[0]):
            if photo_check_err[j][k] > 0 and photo_check[j][k] > 3 * photo_check_err[j][k]:
                mag_check[j][k] = mag_ref[j] - 2.5 * math.log10(photo_check[j][k] / photo_ref[j])
    return mag_check

def magnitude_check_ref(mag_ref, photo_ref, photo_check, photo_check_err):
    mag_check = np.empty_like(photo_check, dtype=float)
    mag_check_err = np.empty_like(photo_check, dtype=float)
    # print(np.log10(photo_source / photo_ref))
    for j in range(len(photo_check)):
        if isnan(photo_check[j]):
            mag_check[j] = np.nan
            mag_check_err[j] = np.nan
        else:
            mag_check[j] = mag_ref - 2.5 * math.log10(photo_check[j] / photo_ref)
            mag_check_err[j] = float(photo_check_err[j]) / photo_check[j]
        #print(mag_check_err[j])
    return mag_check, mag_check_err
...
def magnitude_err(photo_source, photo_err, mag_check, check_photo, check_err):
    ...
    for k in range(mag_check.shape[1]):
        ...
        for j in range(mag_check.shape[0]):
            check_total_mag += mag_check[j][k] * (check_photo[j][k] / check_err[j][k]) ** 2
            check_vis_frame += (check_photo[j][k] / check_err[j][k]) ** 2
        if check_vis_frame != 0:
            check_average[k] = check_total_mag / check_vis_frame
        for j in range(mag_check.shape[0]):
...
def adv_magnitude_err(mag_source, mag_check, check_photo, check_err):
    #SNR_check = check_photo / check_err
    check_average = np.zeros(mag_check.shape[1])
    mag_check_var_err = np.empty_like(mag_check)
    mag_check_signal_err = np.empty_like(mag_check)
    mag_source_var_err = np.zeros_like(mag_source)
    source_var_check_num = np.zeros_like(mag_source)
    for k in range(mag_check.shape[1]):
        check_total_mag = 0
        check_vis_frame = 0
        for j in range(mag_check.shape[0]):
            #if k == 27:
            #    print(mag_check[j][k])
            if np.isnan(mag_check[j][k]) == 0:
                check_total_mag += mag_check[j][k] * (check_photo[j][k] / check_err[j][k]) ** 2
                check_vis_frame += (check_photo[j][k] / check_err[j][k]) ** 2
                #if k == 27:
                    #print(mag_check[j][k], check_total_mag, check_vis_frame)
            #else:
                #print('NaN')
        if check_vis_frame != 0:
            check_average[k] = check_total_mag / check_vis_frame
        ...
        for j in range(mag_check.shape[0]):
            if np.isnan(mag_check[j][k]) == 0:
                mag_check_var_err[j][k] = abs(check_average[k] - mag_check[j][k])
                mag_check_signal_err[j][k] = 1 / (check_photo[j][k] / check_err[j][k])
            else:
                mag_check_var_err[j][k] = np.nan
                mag_check_signal_err[j][k] = np.nan

    # Calculating variability for source
    # print(check_average)

    for j in range(mag_check.shape[0]):
        for k in range(mag_check.shape[1]):
            if np.isnan(mag_check[j][k]) == 0:
                if mag_check[j][k] < mag_source[j] + 0.2 and mag_check[j][k] > mag_source[j] - 0.2:
                    if mag_check_var_err[j][k] <= 0.3:
                        mag_source_var_err[j] += (mag_check[j][k] - check_average[k]) ** 2
                        source_var_check_num[j] += 1
        mag_source_var_err[j] = math.sqrt(mag_source_var_err[j] / (source_var_check_num[j] - 1))
    source_bright_var_check_num = np.zeros_like(mag_source)
    # Variability of 15 brighter checks
    mag_source_bright_var_err = np.zeros_like(mag_source)
    for j in range(mag_check.shape[0]):
        for k in range(mag_check.shape[1]):
            if np.isnan(mag_check[j][k]) == 0:
                if mag_check_var_err[j][k] <= 0.2:
                    mag_source_bright_var_err[j] += mag_check_var_err[j][k] ** 2
                    source_bright_var_check_num[j] += 1
        mag_source_bright_var_err[j] = math.sqrt(mag_source_bright_var_err[j] / (source_bright_var_check_num[j] - 1))
    return mag_source_var_err, mag_check_var_err, mag_check_signal_err, source_var_check_num, check_average, mag_source_bright_var_err

def export_magnitude_correct(photo_dataframe):
    #SNR_check = check_photo / check_err
    source_mag_corr = photo_dataframe['Source mag (corrected)']
    source_mag = photo_dataframe['Source mag']
    exist_check = 1
    check_ind = 1
    check_average = np.empty(0)
    check_average_ind = 0
    while exist_check == 1:
        check_col_name = "Check star " + "{}".format(check_ind) + " mag"
        if check_col_name in photo_dataframe.columns:
            check_total_mag = 0
            check_vis_frame = 0
            for frame_num in range(len(source_mag_corr)):
                mag_check = photo_dataframe[check_col_name]
                check_signal_mag_err = photo_dataframe[check_col_name + " error (signal noise)"]
                if np.isnan(mag_check.values[frame_num]) == 0:
                    check_total_mag += mag_check.values[frame_num] / check_signal_mag_err.values[frame_num] ** 2
                    check_vis_frame += 1 / check_signal_mag_err.values[frame_num] ** 2
            if check_vis_frame != 0:
                check_average = np.append(check_average, check_total_mag / check_vis_frame)
                for frame_num in range(len(source_mag_corr)):
                    if np.isnan(mag_check.values[frame_num]) == 0:
                        photo_dataframe[check_col_name + " error (variability, not corrected)"].values[frame_num] = abs(check_average[check_average_ind] - mag_check.values[frame_num])
                check_average_ind += 1
        else:
            exist_check = 0
        check_ind += 1
    ...
    # Calculating variability for source
    for frame_num in range(len(source_mag_corr)):
        mag_source_var_err = 0
        source_var_check_num = 0
        check_average_ind = 0
        for check_ind in range(1, check_num):
            check_col_name = "Check star " + "{}".format(check_ind) + " mag"
            if check_col_name in photo_dataframe.columns:
                mag_check = photo_dataframe[check_col_name]
                if np.isnan(mag_check.values[frame_num]) == 0:
                    if mag_check.values[frame_num] < source_mag.values[frame_num] + 0.2 and mag_check.values[frame_num] > source_mag.values[frame_num] - 0.2:
                        mag_source_var_err += (mag_check.values[frame_num] - check_average[check_average_ind]) ** 2
                        source_var_check_num += 1
                    check_average_ind += 1
        photo_dataframe["Source mag error (variability of checks, not corrected)"].values[frame_num] = math.sqrt(mag_source_var_err / (source_var_check_num - 1))
    ...
    check_average_ind = 0
    while exist_check == 1:
        check_col_name = "Check star " + "{}".format(check_ind) + "corrected mag"
        if check_col_name in photo_dataframe.columns:
            check_total_mag = 0
            check_vis_frame = 0
            for frame_num in range(len(source_mag_corr)):
                mag_check = photo_dataframe[check_col_name]
                check_signal_mag_err = photo_dataframe["Check star " + "{}".format(check_ind) + " mag error (signal noise)"]
                if np.isnan(mag_check.values[frame_num]) == 0:
                    check_total_mag += mag_check.values[frame_num] / check_signal_mag_err.values[frame_num] ** 2
                    check_vis_frame += 1 / check_signal_mag_err.values[frame_num] ** 2
            if check_vis_frame != 0:
                ...
                for frame_num in range(len(source_mag_corr)):
                    if np.isnan(mag_check.values[frame_num]) == 0:
                        photo_dataframe["Check star " + "{}".format(check_ind) + " mag error (variability, corrected)"].values[frame_num] = abs(
                            check_average[check_average_ind] - mag_check.values[frame_num])
                check_average_ind += 1
        else:
            exist_check = 0
        check_ind += 1
    ...
    for frame_num in range(len(source_mag_corr)):
        ...
        for check_ind in range(1, check_num):
            ...
            if check_col_name in photo_dataframe.columns:
                mag_check = photo_dataframe[check_col_name]
                if np.isnan(mag_check.values[frame_num]) == 0:
                    if mag_check.values[frame_num] < source_mag_corr.values[frame_num] + 0.2 and mag_check.values[
                        frame_num] > source_mag_corr.values[frame_num] - 0.2:
                        mag_source_var_err += (mag_check.values[frame_num] - check_average[check_average_ind]) ** 2
                        source_var_check_num += 1
                    check_average_ind += 1
        photo_dataframe["Source mag error (variability of checks, corrected)"].values[frame_num] = math.sqrt(
            mag_source_var_err / (source_var_check_num - 1))
    return photo_dataframe
...
def full_var_correct(check_mag, check_mag_err, check_true_mag, check_color, check_x, check_y,
                     frame_shape, coord_poly_num, color_param_num, max_param_num):  # Fitting by a 4 order polynom on each coordinate (
    # a0+a1*x+a2*x**2+a3*x**3+a4*x**4) taking into account weights
    ...
    # Fit parameters
    coord_par = np.zeros((photo_len, max_param_num))
    coord_par_err = np.zeros((photo_len, max_param_num))
    # Main body
    for k in range(photo_len):
        ...
        for i in range(check_len):
            ...
            for poly_num in range(1, coord_poly_num + 1):
                for coord_ind in range(poly_num + 1):
                    #print('Poly deg: ' + "{}".format(poly_num - coord_ind) + "; " + "{}".format(coord_ind))
...
def sing_frame_full_var_correct(check_mag, check_mag_err, check_true_mag, check_color, check_x, check_y,
                     frame_shape, coord_poly_num, color_param_num, max_param_num):  # Fitting by a 4 order polynom on each coordinate (
    # a0+a1*x+a2*x**2+a3*x**3+a4*x**4) taking into account weights
    full_param_num = (coord_poly_num ** 2 + 3 * coord_poly_num) / 2 + color_param_num + 1
    print("Number of all parameters: " + "{}".format(full_param_num))
    ...
    for i in range(check_len):
        ...
        for poly_num in range(1, coord_poly_num + 1):
            for coord_ind in range(poly_num + 1):
                x_input_arr[i][place_index] = (check_x[i] - check_mid_x) ** (poly_num - coord_ind) * (
                            check_y[i] - check_mid_y) ** (coord_ind)
                place_index += 1
        coord_var_mag[i] = check_mag[i] - check_true_mag[i]
        #weight_arr[i][i] = 1 / check_mag_err[i] ** 2
        weight_arr[i][i] = 1
    ...
    #    if parameter_ind != x_converted_arr.shape[0] - 1:
    #        for substract_ind in range(parameter_ind + 1, x_converted_arr.shape[1]):
    #            substract_value += coord_par[substract_ind] * x_converted_arr[parameter_ind][
    #                substract_ind]
    #    right_value = y_converted_arr[parameter_ind] - substract_value
    #    coord_par[parameter_ind] = right_value / x_converted_arr[parameter_ind][parameter_ind]

    # Error estimation
    sum_arr = coord_var_mag - np.dot(x_input_arr, coord_par)
    sum_arr_trans = np.transpose(sum_arr)
    minim_sum = np.dot(np.dot(sum_arr_trans, weight_arr), sum_arr)
    #x_error_converted,
    parameter_variance = minim_sum / (check_len - full_param_num) * x_inv_arr
    for m in range(0, full_param_num):
        if parameter_variance[m][m] >= 0:
            ...
            coord_par_err[m] = -math.sqrt(-parameter_variance[m][m])
    minim_sum = np.dot(sum_arr_trans, sum_arr)
    while len(coord_par) < max_param_num:
        coord_par = np.append(coord_par, 0)
        coord_par_err = np.append(coord_par_err, 0)
    print('Sum of deviation: ' + "{}".format(minim_sum))
    print(coord_par)
    return coord_par, coord_par_err
...
def check_mag_correct(check_mag, check_color, check_x, check_y, corr_param, frame_shape, coord_poly_num_arr, color_param_num_arr):
    ...
    for k in range(photo_len):
        color_param_num = int(color_param_num_arr[k])
        coord_poly_num = int(coord_poly_num_arr[k])
...
def source_mag_correct(source_mag, source_color, source_x, source_y, corr_param, frame_shape, coord_poly_num_arr, color_param_num_arr):
...
def obj_centroid(check_x, check_y, radius, gap, annul, ground, back_level):
    ...
    return int(round(x_centre)), int(round(y_centre))

def obj_bright_centre(check_x, check_y, radius, gap, annul, ground, back_level):
    x0 = int(round(check_x))
    y0 = int(round(check_y))
    radius = int(radius)
    ...
    for iter in range(3):
        ...
        for i in range(x_bottom, x_top):
            for j in range(y_bottom, y_top):
                ...
                if ground[i][j] > 0:
                    if (rad_val <= radius and rad_val <= radius + gap):
                        if ground[i][j] >= max_pixel:
                            max_x = i
                            max_y = j
                            max_pixel = ground[i][j]
    return max_x, max_y
...
def BrightStarFinder(frame_data, num_of_bright):
    ...
    while found_stars < num_of_bright:
        search_level = (frame_max - back_level) * (97 - iter_num) / 100 + back_level
        #print(search_level, 'a', frame_max, back_level, iter_num)
        if search_level <=0:
            break
        ...
        if len(poss_star_coord_x) == 1:
            ...
            def_star_photo = fast_photo(def_star_coord_x[0], def_star_coord_y[0],
                                        back_level, frame_data)
            def_star_peak = obj_peak(def_star_coord_x[0], def_star_coord_y[0], 10, search_frame)
            #print(frame_data[def_star_peak[0]][def_star_peak[1]])
            if frame_data[def_star_peak[0]][def_star_peak[1]] >= frame_max * 0.9:
                fill_area(def_star_peak[0], def_star_peak[1], search_frame, back_level)
                continue
            ...
            for poss_ind in range(1, len(poss_star_coord_y)):
                ...
                if distance_check == len(def_star_coord_x):
                    ...
                    def_star_photo = fast_photo(inter_star_coord_x, inter_star_coord_y,
                                                back_level, frame_data)
                    inter_star_peak = obj_peak(inter_star_coord_x, inter_star_coord_y, 10, search_frame)
                    #print(frame_data[inter_star_peak[0]][inter_star_peak[1]])
                    if frame_data[inter_star_peak[0]][inter_star_peak[1]] >= frame_max * 0.9:
                        fill_area(def_star_peak[0], def_star_peak[1], search_frame, back_level)
                        continue
