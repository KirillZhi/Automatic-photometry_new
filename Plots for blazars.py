...
line_point_str = '2022-02-25 14:12:51.1'         # t0 for plots such as GRBs
# Reading a datafile
...
while end_check == 0:
    ...
    for j_ind in range(len(data['Source mag (corrected)'])):
        if abs(float(data['Source mag error (signal noise)'].values[j_ind])) < 0.3 and abs(float(data['Source mag error (variability of bright checks, corrected)'].values[j_ind])) < 0.3:
