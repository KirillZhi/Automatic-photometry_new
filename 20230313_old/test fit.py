...
source_ra, source_dec = 114.53075, 17.705278  # Celestial (equatorial) coordinates of the studied source
...
source_mag_interval = np.array([14, 17])
...
check_true_mag = check_ref_stars['Gmag']

check_color = check_ref_stars['BP-RP']

print('Frames are picked, building the coordinate net')
print('Approximate time till completion: ' + "{}".format(len(photo_frame_path) * 20. / 60 / 12) + ' minutes')
start_time = time()
...
for frame_num in range(len(photo_frame_path)):
    ...
    check_frame = np.where(check_frame < 200000, check_frame, 200000)
    ...
    for i in range(check_len):
        ...
        if abs(check_ref_x[i] - check_mid_x) >= check_mid_x * 0.95 or abs(check_ref_y[i] - check_mid_y) >= check_mid_y * 0.95:
            continue
        if check_frame[int(check_ref_x[i])][int(check_ref_y[i])] != 0 and check_frame[int(check_ref_x[i])][
            ...
            if check_snr >=5:
                ...
                if max_brightness > 50000:
                    continue
                #print(check_x[i], check_y[i], check_snr)
                #print(check_snr)
                if check_snr >= 10:
                    if abs(check_ref_x[i] - check_mid_x) <= check_mid_x * 0.95 and abs(check_ref_y[i] - check_mid_y) <= check_mid_y * 0.95:
    ...
    #print(check_stars['Gmag'])
    #circle_check(check_x, check_y, 5, 10, 20, centered_frame)
    #plt.figure(2)
    #im = plt.imshow(centered_frame)
    #plt.colorbar(im)
    #c_min = mean
...
check_ref_true_mag = zeros((photo_len, check_len))
...
for frame_num in range(photo_len):
    ...
    for i in range(check_len):
        ...
        check_x[frame_num][i], check_y[frame_num][i] = obj_centroid(check_x[frame_num][i],
                                                                            ...
                                                                            15,
                                                                            check_frame, 0)
    check_photo[frame_num], check_ref_photo_err = photometry_check(check_x[frame_num], check_y[frame_num],
                                                                          ap_radius, check_gap,
                                                                          check_annul, check_frame)
...
source_mag, check_mag = magnitude(mag_ref[0], ref_photo, source_photo, check_photo)
#print(photo_limit)
#print(ref_photo)
#print(mag_ref)
limit_mag, check_ref_mag = magnitude(mag_ref[0], ref_photo, photo_limit, check_ref_photo)
#print(limit_mag)
# Deleting apparently overly variable stars
new_check_photo = copy(check_ref_photo)
new_check_x = copy(check_ref_x)
new_check_y = copy(check_ref_y)
new_check_mag = copy(check_ref_mag)
new_check_true_mag = copy(check_ref_true_mag)
new_check_color = copy(check_ref_color)
new_check_photo_err = copy(check_ref_photo_err)
#print(check_mag)
...
for frame_check_ind in range(1):
    for j_ind in range(len(check_photo[0])):
        if abs(check_ref_mag[frame_check_ind][j_ind] - check_ref_true_mag[frame_check_ind][j_ind]) >= 2:
            new_check_ref_photo = delete(new_check_photo, j_ind - del_ind, axis=1)
...
source_mag_var_err, source_mag_signal_err, check_ref_mag_var_err, check_ref_mag_signal_err = magnitude_err(source_photo,
                                                                                                   source_err,
                                                                                                   check_ref_mag,
                                                                                                   check_ref_photo,
                                                                                                   check_ref_photo_err)
...
coord_poly_num = int((-3 + sqrt(1 + 8 * (len(check_mag[0]) - color_param_num))) / 2 - 1)  # Maximum polynom number
...
x_par, x_par_err = full_var_correct(check_mag, check_mag_signal_err,
                                    check_true_mag, check_color, check_x, check_y, frame_shape, coord_poly_num, color_param_num)
...
for m in range(x_par.shape[1]):
    ...
    if m >= color_param_num and m <= coord_par_num:
